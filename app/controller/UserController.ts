...
import Tips from '../constant/handleError'
import {
  IDeleteAdmin,
  IDeleteAdminRules,
  ILogin,
...
import {deleteToken, setToken, tokenSign, tokenValidate, tokenVerify} from '../../utils/token'
...
class UserController {
  ...
  async update(ctx: Context) {
    const {data, error} = await validator<IUpdateAdmin>(ctx, IUpdateAdminRules)
    if (error !== null) {
      return fail(ctx, error)
    }
  ...
  async delete(ctx: Context) {
    const {data, error} = await validator<IDeleteAdmin>(ctx, IDeleteAdminRules)
    if (error !== null) {
      return fail(ctx, error)
    }

    const count: number = await UserService.deleteUserById(data.id as unknown as number) as unknown as number
  ...
  async findAdminById(ctx: Context) {
    ...
    const user: User | null = await UserService.getUserById(data.id as unknown as number)
    if (user == null) {
  ...
  async verify(ctx: Context) {
    ...
    // 验证token
    // @ts-ignore
    const {user, error} = await tokenValidate(token.toString(), username.toString())
    if (user == null) {
      return fail(ctx, error?.message)
    }

    // 验证permission
    const permissions = await UserService.getUserPermissions(username.toString())
